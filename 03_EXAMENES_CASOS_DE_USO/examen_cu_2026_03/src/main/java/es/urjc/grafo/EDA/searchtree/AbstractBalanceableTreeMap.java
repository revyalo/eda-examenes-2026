 ...
 */
public abstract class AbstractBalanceableTreeMap<K, V> extends AbstractSortedMap<K, V> {
    ...
    protected static class BalanceableBinaryTree<K,V> extends LinkedBinaryTree<Entry<K,V>> {
         ...
         * Es decir, hace que child se convierta en el hijo izquierdo o derecho de parent.
         * Concretamente, si makeLeftChild es true, entonces child se convierte en el
         * hijo izquierdo de parent.
         * Si es false, entonces child se convierte en el hijo derecho de parent.
         * */
        ...
        /**
         * Rota la posición position por encima de su padre. Cambia entre estas dos configuraciones,
         * dependiendo de si position es hijo izquierdo o derecho de su padre.
         *<pre>
         *          b                  a
         *         / \                / \
         *        p  t2             t0   b
         ...
         *      t0  t1                 t1  t2
         *</pre>
         * Es decir, si position es hijo izquierdo de su padre, entonces se hace una rotación a la derecha,
         * como se puede ver en la figura. Si position es hijo derecho de su padre,
         * entonces se hace una rotación a la izquierda.
         *
         * Se asume que la posición position tiene un padre. Es decir, que no es la raíz.
         * El método que llame a esta operación debe asegurarse de ello.
         *
         * @param position la posición que se va a rotar con su padre.
         */
        public void rotate(Position<Entry<K,V>> position) {
            ...
            if (z == null) {
                // Este es un caso especial. Si "y" es la raíz del árbol, entonces "x" se convierte en la nueva raíz.
                root = x;
                x.setParent(null);
            } else {
                // En caso contrario, simplemente hacemos que "x" sea hijo de "z".
                relink(z, x, y == z.getLeft());                      // x becomes direct child of z
            }
            // A continuación, realizamos la rotación propiamente dicha.
            if (x == y.getLeft()) {
                // Si "x" es hijo izquierdo de "y", hacemos una rotación a la derecha.
                // Es decir, "y" se convierte en hijo derecho de "x"
                // y el hijo derecho de "x" se convierte en hijo izquierdo de "y".
                relink(y, x.getRight(), true);
                relink(x, y, false);
            } else {
                // Si "x" es hijo derecho de "y", hacemos una rotación a la izquierda.
                // Es decir, "y" se convierte en hijo izquierdo de "x"
                // y el hijo izquierdo de "x" se convierte en hijo derecho de "y".
        ...
        /**
         * Realiza una reestructuración trinodo de la posición x con su padre y abuelo.
         * Devuelve la posición que se convierte en la raíz del subárbol reestructurado.
         * Asume que los nodos están en una de las siguientes configuraciones:
         *<pre>
         *     z=a                 z=c           z=a               z=c
         *    /  \                /  \          /  \              /  \
         ...
         *</pre>
         * En cualquier caso, el subárbol se reestructurará de modo que el nodo con clave b se convierta en su raíz.
         *<pre>
         *           b
         *         /   \
         *       a       c
         ...
         *</pre>
         *
         * Se asume que la posición x tiene un abuelo. El método que llame a esta operación debe asegurarse de ello.
         *
         * @param x la posición que se va a reestructurar con su padre y abuelo.
         * @return la posición que se convierte en la raíz del subárbol reestructurado.
         */
        public Position<Entry<K,V>> restructure(Position<Entry<K,V>> x) {
            Position<Entry<K,V>> y = parent(x);
            Position<Entry<K,V>> z = parent(y);
            if ((x == right(y)) == (y == right(z))) {
                // Caso 1: x e y están alineados (ambos son hijos izquierdos o ambos son hijos derechos)
                // Se realiza una única rotación sobre y
                rotate(y);
                return y;
            } else {
                // Caso 2: x e y no están alineados (uno es hijo izquierdo y el otro es hijo derecho)
                // Se realizan dos rotaciones: primero sobre x y luego sobre x de nuevo
                rotate(x);
                rotate(x);
                return x;
            }
    ...
    public AbstractBalanceableTreeMap() {
        super();
        tree.addRoot(null);
    }
     ...
     */
    public AbstractBalanceableTreeMap(Comparator<K> comp) {
        super(comp);
        tree.addRoot(null);
    }

    protected Position<Entry<K,V>> restructure(Position<Entry<K,V>> x) {
        return this.tree.restructure(x);
    }
    ...
    public V remove(K key) {
        ...
        if (isExternal(position)) {
            ...
            Position<Entry<K,V>> hermano = sibling(leaf);
            tree.remove(leaf);
