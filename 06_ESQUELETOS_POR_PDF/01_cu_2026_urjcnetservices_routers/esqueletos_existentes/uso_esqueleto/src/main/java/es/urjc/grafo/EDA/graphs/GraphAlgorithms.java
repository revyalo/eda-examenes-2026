...
public class GraphAlgorithms {
    ...
    /**
     * Calcula el árbol de expansión a partir del vértice source en el grafo g.
     * @param g grafo del que se quiere calcular el árbol de expansión.
     * @param source vértice desde el que se quiere calcular el árbol de expansión.
     * @param <V>
     * @return árbol de expansión.
     */
    public static <V> Tree<Vertex<V>> expansionTree(AdjacencyMapGraph<V,?> g, Vertex<V> source) {
        Set<Vertex<V>> known = new HashSet<>();
        LinkedTree<Vertex<V>> tree = new LinkedTree<>();
        Position<Vertex<V>> rootPos = tree.addRoot(source);
        known.add(source);
        Queue<Position<Vertex<V>>> queue = new LinkedList<>();
        queue.add(rootPos);
        while (!queue.isEmpty()) {
            Position<Vertex<V>> currentPos = queue.poll();
            Vertex<V> currentVertex = currentPos.getElement();
            for (Vertex<V> reachableVertex : g.reachableVertices(currentVertex)) {
                if (!known.contains(reachableVertex)) {
                    known.add(reachableVertex);
                    Position<Vertex<V>> newPos = tree.add(reachableVertex, currentPos);
                    queue.add(newPos);
                }
            }
        }
