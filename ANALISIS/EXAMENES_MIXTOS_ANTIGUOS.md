...
- Las partes de implementación se han transformado en proyectos bajo `IMPLEMENTACION`.
- Las partes de dominio se han transformado en proyectos bajo `CASOS_DE_USO`.
- Los iteradores se han separado en `ITERADORES` cuando convenía estudiarlos de forma independiente.
...
La organización antigua por PDF se consolidó para no duplicar material en el directorio raíz.
Los proyectos útiles están ahora en `IMPLEMENTACION`, `CASOS_DE_USO` e `ITERADORES`.
